...
% Modified for use with LCNI stimulus presentation boxen (MATLAB2012b on 32-bit Windows XP)
...
if MRI==1,
    ...
    %green = KbName('g');
    %red = KbName('r');
    %response_codes=['y' 'b'];   % for scanner with button box put on setting 2--for eastern and northern respectively
    if which_box==1,
        nyx = KbName('3');
        zur = KbName('2');
...
%     if MRI==1,
%             % Try 1 to use new trigger (if key is pressed and that key is
%             % "t" from the trigger and no response yet, then...)
% %             if keyIsDown & keyCode(trigger) & noresp,
% %                 noresp=0;
% %             end;
%             % Try 2 to use the new trigger (if a key is pressed and no
%             % response yet, if you compare strings and the keycode is t,
%             % then...)
%             if keyIsDown & noresp,
...
%                 end;
%             end;
%             % Try 3 to use the new trigger (using GetChar)--CAUSES COMPUTER
%             % TO CRASH! DOESN'T WORK!
% %             ListenChar;
% %             FlushEvents;
% %             while GetChar ~= 't',
% %             end;  
...
try,  %part of try...catch (line 302) 
    ...
    for trial=1:ntrials, % start trial loop
        ...
        while noresp,
            ...
            if MRI==1,
                if keyIsDown & noresp,
                        % Used to ignore trigger, but instead can disable
                        % trigger (line 246) and then collect responses
                        % normally
%                     if ~keyCode(trigger),
%                         tmp=KbName(keyCode);
...
%                         noresp=0;
%                     elseif keyCode(trigger) & (keyCode(blue) | keyCode(yellow) | keyCode(green) | keyCode(red)),
%                         keyCode(trigger)=0;
%                         tmp=KbName(keyCode);
%                         rec_task(trial_counter,6)={tmp(1)};
%                         rec_task(trial_counter,7)={GetSecs-start_time};
