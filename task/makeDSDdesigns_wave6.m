...
neutStatementFile = 'materials/statements_neutral_wave4and5.txt';
affStatementFile = 'materials/statements_affect_wave4and5.txt';
...
for dCount = 1:NSubsTotal
    ...
    neutStatements = neutRawStatements(randperm(length(neutRawStatements)));
    affStatements = affRawStatements(randperm(length(affRawStatements)));
             
    shuffLeftIsMorePairs=leftIsMorePairs(randperm(length(1:14)),:);
    shuffRightIsMorePairs=rightIsMorePairs(randperm(length(1:14)),:);
    shuffEqPairs=eqPairs(randperm(length(1:14)),:);

    for rCount = 1:2;
        thisRun = (['run',num2str(rCount)]);
        rawJitter = rawJitter(randperm(length(rawJitter)),:);
        discoJitter = discoJitter(randperm(length(discoJitter)),:);
        zCount = 0;
...
for dCount = 1:NSubsTotal
    waveNum = floor(((dCount-1)/NRealSubsTotal)+6); % wave number is just how far you are in the count
    
    subIDNum = dCount - NRealSubsTotal * (waveNum - 6);
    ...
    for rCount = 1:2
        ...
        for tCount = 1:numOptTrials
          fid = fopen([targetDirectory,'/',subID,'_wave_',num2str(waveNum),'_dsd_','run',num2str(rCount),'_input.txt'],'a');
